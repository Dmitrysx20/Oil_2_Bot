...
class SmartRouter {
  constructor() {
    ...
    this.medicalSymptoms = {
      ...
      'суставы': ['болит колено', 'колено болит', 'колено', 'колени', 'сустав', 'суставы', 'артрит', 'артроз', 'растяжение', 'ушиб'],
      'женское_здоровье': ['пмс', 'предменструальный синдром', 'менструальные боли', 'боль при месячных', 'спазмы при месячных', 'дисменорея']
    };
  ...
  checkMedicalSymptoms(normalizedText) {
    ...
    // Упрощённое распознавание боли по шаблону "болит <часть тела>"
    if (normalizedText.startsWith('болит ') || normalizedText.includes(' болит ')) {
      const after = normalizedText.split('болит').pop().trim();
      const symptom = after.length > 0 ? `болит ${after}` : 'боль';
      return { isMedical: true, type: 'symptom', symptom, category: 'общий_вопрос' };
    }
