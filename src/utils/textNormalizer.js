...
const normalizationDictionary = {
  ...
  'аромамасло': 'масло',
  'аромамасла': 'масла',
  // Частые варианты написаний масел/словосочетаний
  'береза': 'берёза',
  'черный перец': 'чёрный перец',
  'перец черный': 'чёрный перец',
  'черная ель': 'чёрная ель',
  'кедер': 'кедр',
  'лемонграс': 'лемонграсс',
...
function fixCommonTypos(text) {
  ...
  corrected = corrected
    ...
    .replace(/ъ/g, 'ь')
    .replace(/э/g, 'е')
    .replace(/черный/g, 'чёрный')
    .replace(/черная/g, 'чёрная');
