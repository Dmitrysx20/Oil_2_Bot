  ...
  async processUpdate(update) {
    try {
      ...
      switch (routeResult.requestType) {
        ...
        case 'aroma_guide':
          return await this.handleAromaGuide(routeResult);
...
Или используйте команду /help для справки.`;
  ...
  async handleAromaGuide(routeResult) {
    const { chatId } = routeResult;
    const message = `Ароматерапия для здоровья\n\n` +
`Ароматерапия — это использование эфирных масел для поддержания здоровья и улучшения самочувствия. Ниже — краткие рекомендации по применению (дозировки, способы и безопасность).\n\n` +
`1) Лаванда\n` +
`- Ингаляции: 2–3 к. на 1 л горячей воды\n` +
`- Массаж: 5–6 к. на 30 мл базового масла\n` +
`- Ванна: 5–10 к. с эмульгатором (молоко/мёд 1 ст.л.)\n` +
`Безопасность: неразбавленно не наносить; глаза избегать; беременность — по согласованию\n\n` +
`2) Чайное дерево\n` +
`- Точечно: 1–2 к. на зону\n` +
`- Ингаляции: 2–3 к. на 1 л\n` +
`Безопасность: тест на чувствительность; лучше в разбавлении\n\n` +
`3) Мята перечная\n` +
`- Ингаляции: 2–3 к. на 1 л\n` +
`- Массаж: 2–3 к. на 30 мл\n` +
`- Диффузор: 4–6 к.\n` +
`Безопасность: детям <2 лет не применять; при чувствит. к ментолу — избегать\n\n` +
`4) Лимон\n` +
`- Ингаляции: 2–3 к. на 1 л\n` +
`- Косметика: 1–2 к. на 10 мл\n` +
`Безопасность: фотосенсибилизация — не перед солнцем; неразбавленно не наносить\n\n` +
`5) Розмарин\n` +
`- Массаж: 5–6 к. на 30 мл\n` +
`- Ингаляции: 2–3 к. на 1 л\n` +
`Безопасность: противопоказан при беременности/эпилепсии; детям <12 лет — избегать\n\n` +
`Общее: соблюдайте дозировки и делайте кожный тест. При хронических заболеваниях — консультация врача.`;
